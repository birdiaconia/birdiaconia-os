...
이 제안서는 현재 저장소에 있는 문서만 근거로 작성했다. 현재 확인할 수 있는 핵심 근거는 `README.md`의 프로젝트 설명이다.

확인한 내용은 다음과 같다.

- 프로젝트 이름은 **Birdiaconia OS**다.
- Birdiaconia OS는 버디아코니아의 설계, 실행, 기록, 자동화, 콘텐츠, 데이터, 의사결정을 관리하기 위한 프로젝트 운영체계다.
- 운영 원칙은 대화를 임시 입력값으로 보고, 결정은 문서로 남기며, 작업은 Issue로 나누고, 실행은 담당자에게 배정하며, 결과는 Pull Request 또는 문서 업데이트로 반영하는 것이다.
- 기본 구조는 방향과 원칙, 운영 흐름, 데이터 모델과 AppSheet 구조, GPT·Codex·AppSheet 등의 역할 정의, 실행 프로젝트, 사례와 참고 자료를 담는 영역으로 구성된다.

## 버디아코니아의 현재 운영 목적

현재 저장소 내용만 기준으로 보면, 버디아코니아에는 다음과 같은 운영체계가 필요하다.
...
- 사례, 자료, 참고 모델을 축적해 조직 지식으로 다시 사용한다.
...
초기 AI 운영 역할은 분야별 전문 업무를 먼저 수행하기보다, 저장소에 정의된 운영체계가 안정적으로 작동하도록 돕는 데 집중해야 한다. 아래 역할은 모두 다음 원칙을 바탕으로 한다.

- 결정은 문서로 남긴다.
- 작업은 Issue로 나눈다.
- 실행은 담당자에게 배정한다.
- 결과는 Pull Request 또는 문서 업데이트로 반영한다.
...
### 역할 목적
문서화된 목표와 결정을 실행 가능한 Issue로 나누어, 담당자 배정과 실행이 가능하도록 만든다.
...
### 역할 목적
GPT, Codex, AppSheet 등 AI와 자동화 역할을 명확히 정의해, AI 업무가 정해진 범위 안에서 수행되고 검토 가능하며 Birdiaconia OS와 일치하도록 한다.
...
### 역할 목적
사례, 자료, 참고 모델을 보존해 버디아코니아가 조직 지식을 다시 사용할 수 있게 한다. 단, 참고 자료와 승인된 결정을 혼동하지 않도록 관리한다.
